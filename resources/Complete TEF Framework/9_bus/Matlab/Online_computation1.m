...
if Shed==1
 ...
 Psh_max=0.60;
 max_iter=20;
data_time=faultclearing_time+0.05;
[~, idx_online] = min(abs(T - data_time));
wpred=w(idx_online,:);
...
VKE(k)=Calculate_KE_online(g, H1, Ws, wpred, MOD);
Vtot(k)=VPE(k)+VKE(k);
Vsh(k)=Vcr-Vtot(k);
...
while k<max_iter
    if Vsh(k)<=threshold
        ...
        % Secant Method for Interpolation:
            ...
            if P_new <= x2
                P_new = x2 + 0.01; % Force 5% increment if math fails
            end
            k=k+1;
            Psh(k)=P_new;
            H1=H;
...
%%
            ...
            Vtot(k)=VPE(k)+VKE(k);
            Vsh(k)=Vcr-Vtot(k);
            fprintf('Iter %d: Shed %.2f%% -> Margin Vsh = %.4f\n', k, Psh(k)*100, Vsh(k));
    end
end
