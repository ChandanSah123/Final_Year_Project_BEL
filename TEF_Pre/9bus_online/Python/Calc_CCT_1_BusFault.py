...
# ==============================================================================
# 1. SETUP & CONFIGURATION (BUS FAULT)
# ==============================================================================
work_dir = config.WORK_DIR
result_dir = config.RESULT_DIR
raw_file = config.RAW_FILE
dyr_file = config.DYR_FILE
out_file = config.OUT_FILE.replace('.out', '_CCT_Bus.out') # Distinct file name

# ==============================================================================
# 2. SIMULATION ENGINE (BUS FAULT - NO TRIP)
# ==============================================================================
def run_simulation(clearing_duration):
    ...
    if 'time' not in chandata or len(chandata['time']) == 0: return False, 999.0
    
    # Assuming standard channel ordering (1,2,3 are Angles)
    angle_keys = [1, 2, 3]
...
# ==============================================================================
print("\n=== FINDING CCT: BUS FAULT (NO TRIP) ===")
psspy.report_output(6, "", []); psspy.progress_output(6, "", [])

t_min, t_max = 0.100, 1.000 # Bus faults usually have higher CCT, expanded range
precision = 0.0001
