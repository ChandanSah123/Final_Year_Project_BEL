...
out_file = config.OUT_FILE.replace('.out', '_Line50Fault.out')

print("--- Simulation: 50% Line Fault (Trip) ---")
...
psspy.chsb(0, 1, [-1, -1, -1, 1, 7, 0]) # Speeds
psspy.chsb(0, 1, [-1, -1, -1, 1, 2, 0]) # Power
...
print(f"Applying 50% Fault Proxy at Bus {config.TRIP_LINE_FROM}...")
# NOTE: Using bus fault as proxy since 50% split bus is not in RAW file.
...
axes[0].set_title("Rotor Angle (50% Line Fault)")
axes[0].legend()
axes[0].grid(True)
