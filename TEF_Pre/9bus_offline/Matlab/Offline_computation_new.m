...
% 1. Point to the new function
fun = @(theta) Integrateth(0, theta, Pi, C, D, H);

% 2. ENABLE Jacobian in Options (Crucial Step!)
options_opt = optimoptions('fsolve', ...
    'Display', 'iter', ...
    'SpecifyObjectiveGradient', true, ...  % <--- THIS IS THE FIX
    'Algorithm', 'levenberg-marquardt', ... % More robust for ridge problems
    'FunctionTolerance', 1e-8, ...
    'StepTolerance', 1e-8, ...
    'MaxIterations', 100);

% 3. Run Solver
fprintf('Initial Guess Gen 1: %.10f\n', theta_u(1)); % Verify consistency
[x_sol, fval, exitflag] = fsolve(fun, theta_u, options_opt);
theta_cuep_mod = x_sol - (sum(x_sol .* H) / sum(H));
% ... [Rest of your code] ...
